...
class BaseDataset(torchdata.Dataset):
    def __init__(self, list_sample, opt, max_sample=-1, split='train'):
        ...
        if isinstance(list_sample, str):
            ...
            self.list_sample = []
            for row in csv.DictReader(open(list_sample, 'r'), delimiter=','):
                if len(row) < 2:
                    continue
        ...
        if self.split == 'train':
            ave_file = '/home/prj/data/AVE_Dataset/trainSet.txt'
        elif self.split == 'val':
            ave_file = '/home/prj/data/AVE_Dataset/valSet.txt'
        else:
            ave_file = '/home/prj/data/AVE_Dataset/valSet.txt'
    ...
    def _load_audio(self, path, center_timestamp, nearest_resample=False):
        ...
        center = int(center_timestamp * self.audRate) # 55000 => 센터프레임스텝 5로 고정을 시켜놨어 ave에서
